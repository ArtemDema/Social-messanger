    ...
    def get(self, request, id):
        ...
        if chat is not None and chat.is_group:
            ...
            for u in online_users:
                if hasattr(u, 'id'):
                    raw_online_ids.append(u.id)
                else:
                    raw_online_ids.append(u)

            for user in chat.users.all():
                users_id.append(int(user.id))
                users_name.append(str(user.first_name))
                
                if user.id in raw_online_ids:
                    online_users_id.append(int(user.id))
            
            group_admin_data = {
                "id": chat.admin.id if chat.admin else None
            }
            current_user_data = {
                "id": request.user.id
            }
